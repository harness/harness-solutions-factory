// Drift analysis pipeline for detecting infrastructure configuration changes
// Pipeline that performs drift analysis to detect infrastructure configuration changes
resource "harness_platform_pipeline" "Execute_Drift_Analysis" {
  lifecycle {
    precondition {
  ...
  yaml = templatefile(
    "${path.module}/templates/pipelines/Execute_Drift_Analysis.yaml",
    {
      ...
      TAGS : yamlencode(merge(local.common_tags, { purpose : "drift_detection" }))
